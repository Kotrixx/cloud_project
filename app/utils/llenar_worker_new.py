...
def configurar_iptables(cliente, vlan_networks):
    ...
    for i, red_origen in enumerate(vlan_networks):
        for red_destino in vlan_networks[i + 1:]:
            comandos += [
                (f"iptables -A FORWARD -s {red_origen} -d {red_destino} -j ACCEPT",
                 f"Permitido el tráfico de {red_origen} a {red_destino}..."),
                (f"iptables -A FORWARD -s {red_destino} -d {red_origen} -j ACCEPT",
                 f"Permitido el tráfico de {red_destino} a {red_origen}...")
            ]
