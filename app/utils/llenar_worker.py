...
workers_config = {
    ...
    '1': {
        'ip': '10.0.0.30',
        'bridge': 'br-int-w1',
        'interfaces': ['tap-w1-vm1-1', 'tap-w1-vm1-2','tap-w1-vm2-1','tap-w1-vm2-2'], #se crean 2 itnerfaces para CADA UNA DE las 2 vms
...
def ejecutar_comandos_worker1(cliente, config):
    comandos = [
        ...
        #vm1
        (f"qemu-system-x86_64 -enable-kvm -vnc 0.0.0.0:1 -netdev tap,id={config['interfaces'][0]},ifname={config['interfaces'][0]},script=no,downscript=no -device e1000,netdev={config['interfaces'][0]},mac={config['mac_addresses'][0]} -netdev tap,id={config['interfaces'][1]},ifname={config['interfaces'][1]},script=no,downscript=no -device e1000,netdev={config['interfaces'][1]},mac={config['mac_addresses'][1]} -daemonize -snapshot cirros-0.5.1-x86_64-disk.img -cpu host", f"Creando VM en {config['interfaces'][0]}..."),
        
        #vm2
        (f"qemu-system-x86_64 -enable-kvm -vnc 0.0.0.0:2 -netdev tap,id={config['interfaces'][2]},ifname={config['interfaces'][2]},script=no,downscript=no -device e1000,netdev={config['interfaces'][2]},mac={config['mac_addresses'][2]} -netdev tap,id={config['interfaces'][3]},ifname={config['interfaces'][3]},script=no,downscript=no -device e1000,netdev={config['interfaces'][3]},mac={config['mac_addresses'][3]} -daemonize -snapshot cirros-0.5.1-x86_64-disk.img -cpu host", f"Creando VM en {config['interfaces'][2]}..."),



        (f"ovs-vsctl add-port {config['bridge']} {config['interfaces'][0]}", f"Conectando {config['interfaces'][0]} al bridge..."),
        (f"ovs-vsctl add-port {config['bridge']} {config['interfaces'][1]}", f"Conectando {config['interfaces'][1]} al bridge..."),
        (f"ovs-vsctl add-port {config['bridge']} {config['interfaces'][2]}", f"Conectando {config['interfaces'][2]} al bridge..."),
        (f"ovs-vsctl add-port {config['bridge']} {config['interfaces'][3]}", f"Conectando {config['interfaces'][3]} al bridge..."),

        
        (f"ovs-vsctl set port {config['interfaces'][0]} tag={config['tags'][0]}", f"Asignando tag VLAN {config['tags'][0]} a {config['interfaces'][0]}..."),
        (f"ovs-vsctl set port {config['interfaces'][1]} tag={config['tags'][1]}", f"Asignando tag VLAN {config['tags'][1]} a {config['interfaces'][1]}..."),
        
        (f"ovs-vsctl set port {config['interfaces'][2]} tag={config['tags'][1]}", f"Asignando tag VLAN {config['tags'][1]} a {config['interfaces'][1]}..."),
        (f"ovs-vsctl set port {config['interfaces'][3]} tag={config['tags'][2]}", f"Asignando tag VLAN {config['tags'][2]} a {config['interfaces'][3]}..."),
